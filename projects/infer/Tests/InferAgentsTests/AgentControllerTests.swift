...
final class AgentControllerTests: XCTestCase {
    ...
    func testBootstrapLoadsFirstPartyPersonasFromBundledURLs() async throws {
        ...
        XCTAssertEqual(ids, [DefaultAgent.id, AutoAgent.id, ReActAgent.id, "fp.good"])
    ...
    func testBootstrapLoadsUserPersonasFromDirectory() async throws {
        ...
        let ids = c.availableAgents.map(\.id)
        XCTAssertEqual(ids, [DefaultAgent.id, AutoAgent.id, ReActAgent.id, "loaded"])
    }
    ...
    func testRefreshListingsPlacesDefaultFirstAndSortsRest() async {
        ...
        // Expected: Default, then synthetic Auto, then synthetic ReAct, then user (sorted by name), then plugin, then firstParty.
        let ids = c.availableAgents.map(\.id)
        XCTAssertEqual(ids, [DefaultAgent.id, AutoAgent.id, ReActAgent.id, "a-user", "b-user", "a-plug", "z-first"])
    }
