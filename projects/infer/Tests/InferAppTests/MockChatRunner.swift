...
// NOTE ON DUPLICATION: this file is the third copy of `MockChatRunner`
// (alongside `InferAppCoreTests` and `InferSessionTests`). SwiftPM test
// targets cannot depend on other test targets, so sharing it would mean
// promoting it to a real library target that ships in the package. The
// existing two copies set the precedent; a fourth consumer is the point
// at which an `InferTestSupport` target earns its keep.
