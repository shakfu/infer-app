...
- [x] ~~**MCP roots / filesystem permissions abstraction.**~~ Done. `MCPServerConfig.roots: [String]` advertised via the `roots` capability in initialize; `MCPClient` responds to inbound `roots/list` with the configured paths; granted scope surfaces as a Console diagnostic at server start. Caveat: protocol-level advertisement, not OS-level enforcement — non-conformant servers can ignore it. The consent gate runs first.

- [x] ~~**Per-server consent gate in the activation path.**~~ Done. `MCPApprovalStore` (UserDefaults-backed) + `MCPApprovalProvider` callback on `MCPHost.bootstrap`. Default behaviour denies unknown servers with a Console diagnostic; `autoApprove: true` in the server config opts out of the gate; `MCPApprovalStore.approve(serverID:)` is the API a future UI calls. Behaviour change: existing configs need `autoApprove: true` set or a stored approval to register tools.
...
- [x] ~~**In-app MCP server config UI.**~~ Done. `MCPServersSection` in the Agents tab lists every server with status (running / denied / disabled / failed), `Approve` / `Revoke` / `Retry` actions, an expandable detail block (command, configured + advertised roots, tool catalogue, failure message), and a `Reveal folder` / `Reload` pair in the section header. Reload uses `MCPHost.reload` which shuts down running clients and sweeps `mcp.<id>.*` tools out of the registry via `ToolRegistry.unregister(prefixed:)`. JSON files are still authored on disk (no in-app edit yet) — the UI is read+approve, not write.
