...
Carved out of scope when the v1 MCP integration landed (item 11 of `REVIEW.md`). v1 covers stdio + tools-only; the items below are independently shippable and roughly ordered by user-visible payoff.

- [ ] **MCP roots / filesystem permissions abstraction.** MCP's `roots` capability lets the client advertise which directories the server is allowed to operate on; without it, an `fs`-style server has access to anything the user does. Today our `StdioMCPTransport` inherits the parent process's filesystem permissions wholesale. Wire the `client/roots/list` notification + a config-file `roots: [path]` field per server, and surface the resolved roots in the Console at server start so the user sees the granted scope.

- [ ] **Per-server consent gate in the activation path.** Today MCP tools land in the same `ToolRegistry` as builtins and ride the same `requirements.toolsAllow` mechanism — fine for explicit allow-listing, but a user with `toolsAllow: []` (i.e. "all tools") accepts every MCP server's tools by default. Add a per-server consent layer (probably an `MCPHost`-owned set of approved server IDs persisted to `UserDefaults`) so the chat VM can prompt on first activation and remember the answer. Pairs naturally with the human-in-the-loop approval gate work for high-risk tool runs.

- [ ] **MCP resources capability.** Servers expose readable URIs (`resources/list`, `resources/read`, `resources/subscribe`). Map onto the existing `Retriever` shape — an MCP resource list can be one source feeding `AgentContext.retrieve` alongside the local vault. Decode `Resource` + `ResourceContent` shapes (text + blob), extend `MCPClient` with `listResources` / `readResource`, and add a `MCPResourceRetriever` adapter. Subscriptions can come later; one-shot reads cover most cases.

- [ ] **MCP prompts capability.** Servers expose curated prompt templates (`prompts/list`, `prompts/get`). Most useful as an authored-prompts surface in the Agents tab — let a user pick "summarise email thread" from a Slack MCP server's prompt library and have it composed into the agent's system prompt at activation time. Schema is small; UI is the bulk of the work.

- [ ] **MCP sampling capability.** Servers can request the host run an LLM decode on their behalf (`sampling/createMessage`) — used by servers that want to chain their own completions through the user's local model rather than spinning up their own. Extend `MCPClient` to handle the inbound request, route through the active `AgentRunner`, return the completion. Permission model: each sampling request needs explicit user approval (the server is asking the host to spend tokens on its behalf), so this depends on the per-server consent gate above.

- [ ] **HTTP / WebSocket transports.** Stdio is fine for local desktop servers; HTTP/SSE and WebSocket (per the MCP HTTP transport spec) cover hosted servers and dev workflows where a server runs on a different machine. Implement two new `MCPTransport` conformances; `MCPClient` consumes them unchanged. Configuration layer needs `transport: "stdio" | "http" | "websocket"` discriminator on `MCPServerConfig` plus URL + auth fields for the network variants.

- [ ] **In-app MCP server config UI.** Today users edit `~/Library/Application Support/Infer/mcp/<id>.json` files directly. A SwiftUI surface in the Agents tab listing each server (status: running / failed / disabled), exposing edit / disable / restart actions, and offering a "discover servers" hook that drops in known-good config files would close the onboarding gap. Pairs with the consent-gate work — the same view becomes the place to revoke approval.

- [ ] **MCP server hot-reload.** Right now `MCPHost.bootstrap` runs once at app start; a config change requires an app restart. Watch the mcp directory with `DispatchSourceFileSystemObject`, diff registered server IDs against the new file set, gracefully shut down removed servers and launch added ones. Re-registering tools in the `ToolRegistry` is already idempotent in shape but needs a removal API (which doesn't exist yet — `ToolRegistry` is currently append-only).
