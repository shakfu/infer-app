...
- [ ] **TTS barge-in (interrupt by voice).** Gate on `continuousVoice`: while `AVSpeechSynthesizer` is speaking AND voice-loop mode is on, run a lightweight `AVAudioEngine` input-node tap computing RMS per buffer. If level stays above ~-30 dBFS for ≥ 200 ms, treat as interrupt: call `synth.stopSpeaking(at: .immediate)` AND explicitly `startDictation()`. The explicit `startDictation` is required because `stopSpeaking` fires `didCancel`, not `didFinish`, so the existing `onFinish` auto-arm path won't trigger. Use a separate `AVAudioEngine` from `SpeechRecognizer`'s (they don't overlap in time but shared engines fight on tap/format). Wrap as `TTSBargeInMonitor` in `SpeechServices.swift`; `SpeechSynthesizer.speak()` starts it, `didFinish`/`didCancel` stops it. Thresholds are code constants for v1 (no UI). Failure modes: mic permission not granted → silently disable barge-in (loop still works); engine fails → log + disable for the session; self-feedback from external speakers is a known v1 risk (AEC is a big lift, deferred). Closes the voice loop — without this, TTS is rude because the user has to wait through every reply.
