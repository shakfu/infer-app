...
Other native deps come in via SPM and compile from source on first build: `libxlsxwriter` (BSD-2-Clause, used by `xlsx.write` — adds `-lz` link, no fetch script, no binary blob), `CoreXLSX` (Apache-2.0, used by `xlsx.read`; pulls XMLCoder + ZIPFoundation transitively), `SQLiteVec` (MIT, vendored at `thirdparty/SQLiteVec/`, used by the RAG vector store), and two tree-sitter grammars vendored at `thirdparty/tree-sitter-qmd/` (Quarto-flavored markdown — block + inline parsers) and `thirdparty/tree-sitter-python/` (Python parser + the upstream `highlights.scm` query) used by the wiki editor's syntax-highlighting pipeline. Both grammars are first-time fetched by the same `manage.py` cache layer; bumping the pinned commit / tag re-fetches automatically.
...
**Editor** — `STTextView`-backed plain markdown with live syntax highlighting via **tree-sitter**. Two grammars run in parallel: `tree-sitter-markdown` (block) gives heading-level fonts, fenced/pandoc code-block detection, and inline-content ranges; `tree-sitter-markdown-inline` paints emphasis (italic), strong (bold), code spans (monospaced), and inline links (accent + underline). Both reparse incrementally on each keystroke (UTF-16 diff → `InputEdit`), so a multi-page document doesn't redo whole-document work as you type. Fenced code blocks get per-language coloring: ` ```python ` and pandoc `{python}` route through `tree-sitter-python` + the upstream `highlights.scm` query (high-fidelity, scm-driven captures); other languages route to Highlightr (~190 grammars via highlight.js). Type `[[` to get a fuzzy autocomplete popover of pages (Tab/Enter inserts, ↑↓ navigate, Esc dismisses). Cmd-click a `[[wikilink]]` in the editor to follow it; clicks resolve case-insensitively with basename fallback so `[[Page]]` finds `Folder/Page` if there's no top-level `Page`. Auto-saves 1.5 s after typing stops. Cmd+S forces a save. Renames carry inbound wikilinks across siblings; folder moves recursively re-key every page inside.
...
| `reset` | Return the project to fresh-clone state (plus any code changes). Removes `build/` and every auto-fetched artifact under `thirdparty/` — xcframeworks, SQLiteVec, tree-sitter grammars, webassets, `Python.framework` — plus the `manage.py` cache markers. Next `make` re-fetches everything from scratch; re-run `make fetch-python` to opt back into the embedded CPython. Preserves `thirdparty/python-apis/` (curated, not fetched) and the HF model cache (use `clean-mlx-cache` for that; reset deliberately doesn't touch it because it can be 20 GB+). Confirms first |
...
| `fetch-tree-sitter-qmd` | Vendor `thirdparty/tree-sitter-qmd/` (Quarto-flavored markdown grammar from quarto-dev/quarto-markdown — block + inline parsers, used by the wiki editor's syntax-highlighting pass). Pinned to a specific commit via `TSQMD_COMMIT=...` since upstream has no tagged releases yet |
| `fetch-tree-sitter-python` | Vendor `thirdparty/tree-sitter-python/` and stage its `highlights.scm` into `Sources/Infer/Resources/python_highlights.scm` (loaded via `Bundle.module` for tree-sitter-driven Python syntax highlighting in the wiki editor). `TSPY_TAG=...` to bump |
