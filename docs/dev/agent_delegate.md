...
- **Router transcript carryover:** chose option (b) from the sketch — a plain-text "Prior dispatches" scratchpad appended to the user text on iteration ≥ 2. Format is template-family-agnostic; the underlying chat template wraps it normally. No changes to `RunOne`'s closure shape, which keeps the unit-test layer intact.
