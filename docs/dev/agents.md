...
- **Swift-native peer: [Swarm](https://github.com/christopherkarani/Swarm)** (christopherkarani, MIT, requires macOS/iOS 26+ and Swift 6.2). Worth knowing about because it's the closest thing to Infer's composition layer in the Swift ecosystem. Workflow DSL with `.step` (sequential), `.parallel`, `.route` (closure-based), `.repeatUntil`, plus a `@Tool` macro that synthesises JSON tool schemas from struct definitions at compile time, a unified `inferenceProvider` protocol spanning Foundation Models / Anthropic / OpenAI / Ollama / MLX, and durable workflow checkpointing for crash-resumable runs. Different design centre: Swarm is declarative-DAG-first (Workflow as the unit of composition), Infer is single-agent-loop-first with composition layered on top via `CompositionPlan`. Closest analogues: `.step` ≈ `chain`, `.route` (Swift closure picks next agent) ≈ `branch` (declarative predicate). No clean Swarm equivalent of Infer's `delegate` (LLM-router-as-loop) or vice versa for `.parallel` (Infer rejected fan-out as an anti-goal; revisit pending capability gates and provider portability — see `TODO.md`). Not a migration target — platform floor differs and Infer's runner / template-family integration is tighter — but the macro-driven tool definition, unified provider abstraction, durable checkpointing, and markdown-first spec format are individually worth borrowing as their respective subsystems are touched.
